...
void encode_leds(uint8_t *grb, uint8_t *spi) {
    for (int i = 0; i < LED_COUNT; i++) {
        uint8_t *p = spi + i * SPI_BYTES_PER_LED;
        encode_byte(grb[i * 3 + 0], p);
        encode_byte(grb[i * 3 + 1], p + 24);
        encode_byte(grb[i * 3 + 2], p + 48);
    }
}
...
void cleanup(int sig) {
    ...
    printf("\n[ambient_daemon] 종료됨 - LED OFF 처리 완료\n");
    exit(0);
}

int main() {
    int fd = open(DEVICE_PATH, O_RDWR);
    spi_fd = open(SPI_PATH, O_WRONLY);
    ...
    if (spi_fd >= 0) {
        ssize_t ret = write(spi_fd, off_spi, sizeof(off_spi));
        if(ret != sizeof(off_spi)){
            perror("initial off write failed or incomplete");
        }
        usleep(80);
    }

    char mode[32] = "off", brightness_str[16] = "mid";
    int brightness = 40;
    uint8_t grb[LED_COUNT * 3] = {0};
    uint8_t spi_buf[LED_COUNT * SPI_BYTES_PER_LED] = {0};
    uint8_t hue = 0;

    while (1) {
        ioctl(fd, AMBIENT_GET_MODE_STR, mode);
        ioctl(fd, AMBIENT_GET_BRIGHTNESS_STR, brightness_str);
